...
However, these spurious fluctuations mostly average out to zero over a time period of 15 days (see \citet{Cox2017}). This can be achieved by setting the \texttt{NLowPass} parameter in \texttt{GPPFourier\_t()} to $15/dt$ where $dt$ is the sampling interval (Figure \ref{Fig3}). 
